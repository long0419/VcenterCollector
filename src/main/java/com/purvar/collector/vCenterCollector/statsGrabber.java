...
class StatsGrabber implements Runnable {
	...
	private String[] getStats(Object[] info) {
		...
		String host = null ;
		String uuid = null ;
		if(info[0] instanceof HostSystem){
			HostSystem hSystem = (HostSystem)info[0] ;
			uuid = hSystem.getHardware().getSystemInfo().getUuid() ;
			host = hSystem.getServerConnection().getUrl().getHost() ;
		}else if(info[0] instanceof VirtualMachine){
			VirtualMachine vMinstance = (VirtualMachine)info[0] ;
			uuid = vMinstance.getSummary().getConfig().getUuid() ;
			host = vMinstance.getServerConnection().getUrl().getHost() ;
		}
		
		final ArrayList<String> temp_results = new ArrayList<String>();
		final String TAG_NS = appConfig.get("vcsTag") + "." + cluster;
			...
			if(pValues != null) {
                for (PerfEntityMetricBase pValue : pValues) {
                    ...
                    for (int x = 0; vals != null && x < vals.length; x++) {
                        ...
                        String unit =  perfKey.get("unit") ;
                        
                        String graphiteTag;
                        if (instance.equals("")) {
                            // no instance, no period required
//                            graphiteTag = TAG_NS + "." + mobType + "." + meNameTag + "." + key + "." + rollup;
                            graphiteTag =uuid + " " + unit + "##" + host + "##" + TAG_NS + "##" + mobType + "##" + meNameTag + "##" + key + "." + rollup ;
                        } else {
//                            graphiteTag = TAG_NS + "." + mobType + "." + meNameTag + "." + key + "." + instance + "." + rollup;

                            graphiteTag =uuid + " " + unit + "##" + host + "##" + TAG_NS + "##" + mobType + "##" + meNameTag + "##" + key + "." + rollup + "##" + instance  ;
                        }
            ...
            if(mob instanceof Object[] && ((Object[])mob)[0] instanceof ManagedEntity) {
                ...
                if(mob.equals("stop_stats")) {
                    ...
                    logger.info(Thread.currentThread().getName() + " took: " + took + "ms" + ",sent " + total_stats
                            + " stats for " + mob_count + " managed objects stats to Backend");
                    total_stats = 0;
                    mob_count = 0;
                }
                try {
